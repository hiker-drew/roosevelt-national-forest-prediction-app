...
with st.expander("ℹ️ About This Website"):
    st.markdown("""
    This project helps identify which types of trees grow in different areas of Roosevelt National Forest using a machine learning algorithm called CatBoost. Using topographic data such as elevation, slope, proximity to water sources, past fires, and roadways, as well as light availability and wilderness area designations, the model predicts which of 7 forest types is found in any given 30m x 30m patch of land. It's a diverse set of trees in Roosevelt National Forest, from a common tree used for timber such as the Douglas-fir to fire-resistant species like Lodgepole Pine and Ponderosa Pine, streamside Cottonwood and Willow trees, pioneer Aspen that colonize burned areas, subalpine Spruce/Fir found just below the alpine zone where Krummholz trees are found naturally sculpted by harsh winds. The problem is ecologically important because different tree species create distinct habitats that support diverse wildlife communities, making forest type prediction crucial for conservation and management efforts.
